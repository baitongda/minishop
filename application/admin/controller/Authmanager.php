...
class Authmanager extends Common
{
    ...
    public function access()
    {
        ...
        $gid = input('id'); 
        // Menu列表数据
        $menuList = Db::name('Menu')->order('sort asc')->select();
        //获取已有权限
        $auth   = Db::name('UserGroup')->where('id',$gid)->find();
        $rules = $auth['rules'];
        ...
        //顶部菜单
        $menuTop = Db::name('MenuGroup')->select();
        foreach ($menuTop as $k=>$top) {
            foreach ($menuListTree as $key => $value) {
                if($top['id'] == $value['group_id']){
                    $menuTop[$k]['_child'][] = $value;
                }               
            }          
        }
        // 输出赋值
        ...
        $this->assign('gid',$gid);
        $this->assign('group',$group);
        $this->assign('menuListTree',$menuTop);
        return $this->fetch();
    }
    /**
     * 设置访问授权
    ...
    public function setAccess()
    {
        ...
        //权限id数组
        $ids = input('post.ids/a');
        $group_ids = input('post.group_ids/a');
        //转换数组为字符串
        if(is_array($ids)){ 
            $ids = implode(',',$ids);
        }
        ...
        $info = Db::name('UserGroup')->where('id',$gid)->update($data);
        if($info){
