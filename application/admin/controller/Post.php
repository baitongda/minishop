...
class Post extends Common
{
    ...
    public function add()
    {
        if (Request::instance()->isPost()) {
            ...
            $position       = input('post.position/a');// 推荐位值,数组可为多个
            ...
            if ($postid) {
                ...
                // 添加封面图
                if ($coverPath) {
                    $covers = explode(",",$coverPath);
                    foreach ($covers as $key => $value) {
                        if ($value) {
                            $coverList['cover_path_'.$key] = $value;
                        }
                    }
                    $result = insert_key_value('posts.cover',$coverList,$data['uuid']);
                    if(!$result){
                        return $this->error('保存封面失败！');
                    }
                }        
                // 遍历加入数据
                foreach ($categoryIds as $key => $value) {
                    // 添加文章-分类表
                    $TermRelationshipsStatus = Db::name('TermRelationships')->insert(['post_id' => $postid,'term_id' => $value]);
            ...
            // 列表数据
            $allTermList = Db::name('Terms')->select();
            // 列表数据转换成树
            $allTermTree = list_to_tree($allTermList);
            // 输出赋值
            $this->assign('allTermTree',$allTermTree);
            return $this->fetch();
        }
    }
    ...
    public function edit()
    {
        if (Request::instance()->isPost()) {
            ...
            if ($postid) {
                ...
                // 清除历史分类
                Db::name('TermRelationships')->where(['post_id' => $postid])->delete();
                // 添加封面图
                if ($coverPath) {                    
                    $covers = explode(",",$coverPath);
                    foreach ($covers as $key => $value) {
                    ...
                    $result = update_key_value('posts.cover',$coverList,$data['uuid']);
                    if(!$result){
                        return $this->error('保存封面失败！');
                    }
                }            
            ...
            // 分类选中
            $categoryList     = Db::name('TermRelationships')->where('post_id',$id)->select();
            // 列表数据
            $allTermList = Db::name('Terms')->select();
            // 列表数据转换成树
            $allTermTree = list_to_tree($allTermList);
            $listView = tree_to_checkbox($allTermTree);
            // 输出赋值
        ...
        if ($status == 'emptytrash') {
            ...
            if( is_array($ids) ){
                foreach ($ids as $key => $value) {                    
                    $value_id = Db::name('TermRelationships')->where('post_id' ,$value)->value('term_id');                    
                }
            }
            // 清空posts表
            $postsResult = Db::name('Posts')->where('id','in',implode(',', $ids))->delete();           
            // 清空TermRelationships表
            Db::name('TermRelationships')->where('post_id','in',implode(',', $ids))->delete();
            // 清空keyvalue表
            ...
            $value_id = Db::name('TermRelationships')->where('post_id' ,$postids['0'])->value('term_id');            
            // 清空posts表
            $postsResult = Db::name('Posts')->where('id',$postids['0'])->delete();           
            // 清空TermRelationships表
            Db::name('TermRelationships')->where('post_id', $postids['0'])->delete();
            // 清空keyvalue表
    ...
    /**
     * 检查分类绑定的表单是否合法(一个文章不能同时属于2个绑定了不同表单的分类)
     * @author tangtanglove
     */
    protected function checkBindForm($categoryIds)
    {
        // 获取uuid数组
        $formList    = Db::name('Terms')->where('id','in',$categoryIds)->column('bind_form');       
        $bingForm    = '';
        foreach ($formList as $key => $value) {
            // 绑定的信息不为空，赋值
            if(!empty($value)&&empty($bingForm)) {
                $bingForm = $value;
            }            
            if(!empty($bingForm)) {
                if($bingForm != $value) {
    ...
    protected function authEdit()
    {
        ...
        $id = input('id');
        //根据文章id，查找文章所属分类id
        $category_id = Db::name('TermRelationships')->where('post_id',$id)->value('term_id');
