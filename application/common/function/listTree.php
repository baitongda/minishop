...
function tree_to_view($list,$level = 0,$repeat = "—&nbsp;")
{
    ...
    foreach ($list as $key => $value) {        
        if (!empty($value['_child'])) {
            // 有子分类，无操作按钮
            $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_status($value['id'])." value='".$value['id']."'></td><td>".$value['id']."</td><td><a href='".url('edit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['name']."</a></td><td>".$value['slug']."</td><td></td></tr>";
            $data = $data.tree_to_view($value['_child'],$level+1);
        } else {
            // 无子分类，有操作按钮
            $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_status($value['id'])." value='".$value['id']."'></td><td>".$value['id']."</td><td><a href='".url('edit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['name']."</a></td><td>".$value['slug']."</td><td><a type='button' class='btn btn-sm btn-danger confirm ajax-get' href='".url('setstatus',['ids'=>$value['id'],'status'=>'delete'])."'>删 除</a></td></tr>";            
        }
    }
    return $data;
}
...
{
    $hasChild = Db::name('Terms')->where(['pid'=>$id])->find();
    if ($hasChild) {
        return 'disabled';
    } else {
...
function tree_to_menu_view($list,$level = 0,$repeat = "—&nbsp;")
{
    ...
    foreach ($list as $key => $value) {
        ...
        $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_menu_status($value['id'])." value='".$value['id']."'></td><td><a href='".url('edit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['name']."</a></td><td>".$value['url']."</td><td> [".$value['gname']."]</td><td>".$value['icon']."</td><td>".$value['sort']."</td><td>".$hide."</td></tr>";
        if (!empty($value['_child'])) {
...
function tree_to_nav_view($list,$level = 0,$repeat = "—&nbsp;")
{
    ...
    foreach ($list as $key => $value) {
        $value['hide'] ? $hide = '是' : $hide = '否';
        $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_nav_status($value['id'])." value='".$value['id']."'></td><td><a href='".url('edit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['name']."</a></td><td><a href='".url('edit',['id'=>$value['id']])."'>".$value['url']."</a></td><td><a href='".url('edit',['id'=>$value['id']])."'>".$value['icon']."</a></td><td>".$value['sort']."</td><td>".$hide."</td></tr>";
        if (!empty($value['_child'])) {
...
function tree_to_page_view($list,$level = 0,$repeat = "—&nbsp;")
{
    ...
    foreach ($list as $key => $value) {
        ...
        if (!empty($value['_child'])) {
            // 有子页面，无操作按钮
            $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_page_status($value['id'])." value='".$value['id']."'></td><td><a href='".url('edit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['title']."</a></td><td>".$value['name']."</td><td>".get_userinfo($value['uid'],'username')."</td><td>".date('Y-m-d H:i:s',$value['createtime'])."</td><td></td></tr>";
            $data = $data.tree_to_page_view($value['_child'],$level+1);
        } else {
            // 无子页面，有操作按钮
            $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_page_status($value['id'])." value='".$value['id']."'></td><td><a href='".url('edit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['title']."</a></td><td>".$value['name']."</td><td>".get_userinfo($value['uid'],'username')."</td><td>".date('Y-m-d H:i:s',$value['createtime'])."</td><td><a type='button' class='btn  btn-sm btn-danger ajax-get confirm' href=".$url.">删除</a></td></tr>";
        }
    }
    return $data;
}
 ...
 * @author  vaey
 */
function tree_to_auth_access($list,$rules,$group_ids,$level = 0,$repeat = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
{
    $data = '';
    if($level>=3){
        $data = '<dd>';
    ...
    foreach ($list as $key => $value) {
        if($level>=3){
            ...
            if (!empty($value['_child'])) {
                $data = $data.tree_to_auth_access($value['_child'],$rules,$group_ids,$level+1);
            }
        }else{
            if($level == 0){
                $str = "<dd class='dd1'>";
            ...
            if($level == 0){
                $data = $data.$str.str_repeat($repeat, $level)."<label><input type='checkbox' name='group_ids' value='".$value['id']."'" .is_checked($value['id'],$group_ids)."> ".$value['name'].' &nbsp;&nbsp;[ 菜单组 ]'."</label></dd>";
            }else{
...
function goods_category_tree_to_view($list,$level = 0,$repeat = "—&nbsp;")
{
    ...
    foreach ($list as $key => $value) {
        ...
        $url = url('goods/categorySetstatus?ids='.$value['id'].'&status=delete');      
        if (!empty($value['_child'])) {
            // 有子分类，无操作按钮
            ...
            // 无子分类，有操作按钮
            $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_goods_status($value['id'])." value='".$value['id']."'></td><td><a href='".url('categoryEdit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['name']."</a></td><td>".$value['slug']."</td><td>".$status."</td><td><a type='button' class='btn btn-sm btn-danger ajax-get confirm' href=".$url.">删除</a></td></tr>";
        }
    }
    return $data;
}     
