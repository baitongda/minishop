...
class Wxpay extends Base
{
	...
	*/
	public function pay()
	{
		...
		import('util.pay.Wxpay.NativePay');
		import('util.pay.Wxpay.Api');
		import('util.pay.Wxpay.WxPayConfig');
		import('util.pay.Wxpay.Data');
		import('util.pay.Wxpay.Exception');
		import('util.pay.Wxpay.Notify');
		...
		if (!empty($postStr)){
			...
			if($ordersResult && $ordersStatusResult){
				...
				foreach ($goodsList as $key => $value) {
					// 销量增加
					Db::name('goods')->where('id',$value['goods_id'])->setInc('sell_num', $value['num']);
					// 库存减少
					Db::name('goods')->where('id',$value['goods_id'])->setDec('num', $value['num']);
					// 用户积分增加
					$goodsInfo = Db::name('goods')->where('id',$value['goods_id'])->find();
					Db::name('users')->where('openid',$openid)->setInc('integral',$goodsInfo['score']*$value['num']);
					// 改变购物车状态
				...
				if(config('web_allow_ticket')){
		            ...
		            $uuid         = "default" ;
		            $name         = "web_print_times";
		            $print        = find_key_value($collection,$uuid,$name);
		            $print        = intval($print);
					for($i=0;$i<$print;$i++){
